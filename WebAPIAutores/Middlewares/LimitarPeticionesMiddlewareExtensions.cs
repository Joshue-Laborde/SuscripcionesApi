...
namespace WebAPIAutores.Middlewares
{
    ...
    public class LimitarPeticionesMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext, ApplicationDbContext context)
        {
            ...
            configuration.GetRequiredSection("limitarPeticiones")
                .Bind(limitarPeticionesConfiguracion);

            var ruta = httpContext.Request.Path.ToString();
            var estaLaRutaEnListaBlnaca = limitarPeticionesConfiguracion.ListaBlancaRutas.Any(x => ruta.Contains(x));

            if (estaLaRutaEnListaBlnaca)
            {
            ...
            if (llaveStringValues.Count == 0)
            {
                ...
                await httpContext.Response.WriteAsync("Debe proveer la llave ne la caercera X-Api-Key");
                return;
            }
            if (llaveStringValues.Count > 1)
            {
                httpContext.Response.StatusCode = 400;
                await httpContext.Response.WriteAsync("Solo una llave de estar presente");
                return;
            }
